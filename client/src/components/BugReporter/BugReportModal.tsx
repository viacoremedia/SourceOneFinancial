...
const S = {
  ...
  toggleBtn: (active: boolean, color: string) => ({
    ...
    background: active ? `${color}25` : 'var(--bg-surface-hover)',
  ...
  severityCard: (active: boolean, borderColor: string) => ({
    ...
    borderRadius: '8px',
    cursor: 'pointer',
    border: `2px solid ${active ? borderColor : 'var(--border-default)'}`,
    background: active ? `${borderColor}25` : 'var(--bg-surface-hover)',
    transition: 'all 150ms',
  }),
  severityLabel: { fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)' },
  severityDesc: { fontSize: '10px', color: 'var(--text-muted)', marginTop: '2px' },
  contextBar: {
    borderRadius: '8px',
    border: '1px solid var(--border-subtle)',
    overflow: 'hidden',
