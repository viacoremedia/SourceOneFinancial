...
async function test() {
  ...
  const server = app.listen(3096, async () => {
    ...
    const resBruce = await fetch('http://localhost:3096/api/analytics/historical/mom?rep=Bruce').then(r => r.json());
    console.timeEnd('Bruce Query');

    console.time('George Query');
    const resGeorge = await fetch('http://localhost:3096/api/analytics/historical/mom?rep=George').then(r => r.json());
